...
- **allabolag.se lookup** by company name: org number, registered län/kommun, basic financials (revenue band, employee count, result). Used to filter (target small, financially-stable, less-contested firms). *Automated fetch is cautious* — allabolag sits behind bot protection; on 403/challenge/parse-fail, silently flag `needs_manual_lookup` and let the user paste. Rate-limit, back off, cache aggressively, sane User-Agent.
